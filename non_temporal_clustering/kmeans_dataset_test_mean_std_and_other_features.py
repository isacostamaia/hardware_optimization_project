...
df = df.groupby('alias').agg({'per_CPU_use':[('mean','mean'),('std','std'),('max','max')]}).dropna()
df
...
plt.xlabel('mean')
plt.ylabel('max')
plt.scatter(X[:,0],X[:,2], label='True Position')

for label, x, y in zip(labels, X[:, 0], X[:, 2]):
    plt.annotate(
        label,
        xy=(x, y), xytext=(-3, 3),
        textcoords='offset points', ha='right', va='bottom')
...
a.scatter(X[:,0], X[:,1], X[:,2], c=kmeanModel.labels_,  cmap='rainbow')
a.set_xlabel('mean')
a.set_ylabel('std')
a.set_zlabel('max')
