...
# +
df0 = df0.groupby('alias').agg({'per_CPU_use':[('mean','mean'),('std','std'),('max','max')]}).dropna()
...
for key in dic.keys():
    ...
    df_nkmeans_group_of_int = df_nkmeans[df_nkmeans.alias.isin(alias_of_int)]
    save_a_cluster_small_single_alias_rolling_mean(df,df_nkmeans_group_of_int,folder_name='Atemporal Clustering_discretizing_cluster_2_METHOD_WARD',note= 'Cluster2_{0}'.format(key))
    figures_to_html_stacked_and_side_by_side_from_folder(folder_name='Atemporal Clustering_discretizing_cluster_2_METHOD_WARD/Cluster2_{0}'.format(key), filename='Atemporal Clustering_discretizing_cluster_2_METHOD_WARD/Cluster2_{0}.html'.format(key))
