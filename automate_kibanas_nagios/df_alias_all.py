...
def make_df_all_alias(df_sv_from_db=False,mode=False):
       
    pd.options.mode.chained_assignment = None  # default='warn'

    if(df_sv_from_db==True):
        df_sv = get_treated_df_sv(mode=mode)
    else:
    ...
    print('Computing treated data per alias...')
    list_df_alias = []
    for os in oss:
        #Machines of the same os that are in Nagios but are not in JSON file
        rest_alias = np.setdiff1d(df_sv[df_sv.alias==os].alias.unique(),release_machines.machine.unique())
        for group in groups:
